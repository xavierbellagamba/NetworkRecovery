...
class segment
{
	//All properties are stored in a private mode
        private :
                int year_, n_cnct_ , n_pts_, db_ID_, unik_ID_, raster_ID_, GMraster_ID_;
		std::vector< std::vector<int> > SPasset_connections_;
		std::vector<int> connected2source_;
		std::vector<int> n_failures_;
		std::vector< std::vector<int> > list_fx_;
                float length_, CRR_estim_;
		std::vector<double> center_loc_;
		std::vector< std::vector<double> > points_;
                std::string SPasset_type_;

	//Set-get methods in public (use of the overload for simplification of the command)
        public :
                //Set methods
		void n_pts(int n) {n_pts_ = n;}
		void n_failures(std::vector<int> n_br) {n_failures_ = n_br;}
		void connected2source(std::vector<int> c2p) {connectedsource_ = c2p;}
		...
		void SPasset_connections(std::vector< std::vector<int> > connect_list) {SPasset_connections_ = connect_list;}
		void list_fx(std::vector< std::vector<int> > l_fx) {list_fx_ = l_fx;}
		...
		void SPasset_type(std::string t) {SPasset_type_ = t;}

		//Get methods
		int n_pts() const {return n_pts_;}
		int n_cnct() const {return n_cnct_;}
		std::vector<int> n_failures() const {return n_failures_;}
		std::vector<int> connected2source() const {return connected2source_;}
		...
		std::vector< std::vector<int> > SPasset_connections() const {return SPasset_connections_;}
		std::vector< std::vector<int> > list_fx() const {return list_fx_;}
		float length() const {return length_;}
		std::vector< std::vector<double> > points() const {return points_;}
		std::string SPasset_type() const {return SPasset_type_;}
