...
class pipeNetwork
{
        ...
        private :
                int n_pipes_, status_year_, network_ID_, n_pumps_, prep_lvl_;
		...
		void pipe_frag_fx(std::vector<int> fx){pipe_frag_fx_ = fx;}
		...
		void pipe_belong(std::vector< std::vector<int> > n) {pipe_belong_ = n;}
		...
		std::vector<int> pipe_frag_fx() const {return pipe_frag_fx_;}
